...
def admins_chat_FILTER(uid, text, teamchat):
    ...
    if any((text.startswith('!'), text.startswith('/'),
            es.exists('saycommand', args[0]), args[0] == 'motd')):
        return default
    ...
    # Special trigger for Global or Admins only message
        ...
        text = text.strip('@').strip()
        if teamchat:
            sam.msg.tell('#admins', '%s #white: #green%s' %
                        (team_tags[team] + user.name, f(text)), False, '#white@#grayADMINS')
        else:
    ...
    if team == 1:
        if teamchat:
            sam.msg.tell(users, '#default(%s) %s%s #white: %s' %
                                (team_names[team], team_tags[team], user.name, f(text)), False)
            return empty
        if alltalk:
            users.append('#human')
        sam.msg.tell(users, '#default*SPEC* %s%s #white: %s' %
    ...
    # If user is in either one of the Teams
        ...
        text = '#default%s%s%s%s%s #white: %s' % ('*DEAD* ' if user.isdead else '',
                                                  ...
                                                  group, team_tags[team], user.name, f(text))
